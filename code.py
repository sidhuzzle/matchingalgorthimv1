...
def matching_algo(Goals,Interest,weight,University,Degree,Subject,Year):
  ...
  df = df_name.set_index(['id', df.groupby('id').cumcount()])['Topic'].unstack().add_prefix('name').reset_index()
  #df_name = pd.DataFrame(df_name.to_records())
  df = pd.merge(df, df_name, left_on='id',right_on='id',suffixes=('', '_x'),how = 'left')
  df = df.loc[:,~df.columns.duplicated()]
  df = df.set_index(['id', df.groupby('id').cumcount()])['Topic'].unstack().add_prefix('Topic').reset_index()
