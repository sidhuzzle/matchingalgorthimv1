...
def matching_algo(Goals,Interest,weight,University,Degree,Subject,Year):
  ...
  df = df.set_index(['id', df.groupby('id','touchpointable_id','type','touchpointable_type','kind','title','creatable_for_name','Weight','city_name','city score','degree score','subject score','year score','value','matching score').cumcount()])['name'].unstack().add_prefix('name').reset_index()
